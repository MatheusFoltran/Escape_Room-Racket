...
(struct Enigma (descricao solucao pistas efeito) #:transparent)
...
(define pedidos(criar-enigma
                ...
                (list "pp1" "pp2" "pp3" "pn1" "pp4" "pn2" "pn3" "pn4")
...
;; ############################################ FUNÇÕES DO ENIGMA PEDIDO ##################################################

(define pp (list "pp4" "pp3" "pp2" "pp1"))  
(define pn (list "pn4" "pn3" "pn2" "pn1"))
(define organizado '())


(define (reset-pedido)
  (set!  pp (list "pp4" "pp3" "pp2" "pp1"))  
  (set! pn (list "pn4" "pn3" "pn2" "pn1"))
  (set! organizado '())
  )



(define (att-pp novo-pp)
  (set! pp novo-pp)
  )

(define (att-pn novo-pn)
  (set! pn novo-pn)
  )



(define (desempilha-pedido pilha)
  (cond 
    [(empty? (rest pilha))
     empty]
    [else
     (append (list (first pilha)) (desempilha-pedido (rest pilha)))]))



(define (empilha-pedido item)
  (set! organizado (append organizado (list item)))
  )


(define (concluido-pedido?)
  (cond
    [(resolver-enigma (Enigma-solucao pedidos) organizado)
     (altera-estado-pedidos)
     (altera-estado-bancada-central)
     ; Inicia o diálogo da descrição do Enigma livro
     (iniciar-sequencia-dialogos (get-narrador) (Dialogo-finais-pedido enigma-feito))]

    [else
     (mostrar-mensagem-dialogo (get-nome-robo) "Os pedidos não estão organizados corretamente. REINICIE O QUIZ!")]
...
(define objetos-armarios
  (list
   (list 150 650 420 100)   ; A1 - Armário inferior esquerdo
   (list 50 370 70 250)     ; A2 - Armário lateral esquerdo
   (list 50 70 70 250)      ; A3 - Armário superior esquerdo
   (list 250 50 420 100)    ; A4 - Armário superior esquerdo
   (list 850 50 420 100)    ; A5 - Armário superior direito
   (list 1380 70 70 250)    ; A6 - Armário superior direito
   (list 1380 370 70 250)   ; A7 - Armário lateral direito
   (list 920 650 420 100)   ; A8 - Armário inferior direito
   (list 670 695 160 55)  ; Porta central
   (list 1060 230 15 15)))
...
(define obj-armario-verm
  (criar-objeto 
   "private armário"
   (list "Achei a chave final! Esse armário estava guardando o que eu mais precisava.")
   (list "Porta"  "Gaveta")) 
  )
...
(define (dialogo-fim)
  (Dialogo-finais
    ...
    (list "Ao abrir uma das gavetas do armário amarelo, você encontra um papel com 'ROOM'. Talvez os enigmas anteriores guardem algo mais... Quem sabe as senhas estejam ligadas às letras do alfabeto?")
